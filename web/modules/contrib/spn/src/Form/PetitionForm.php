...
class PetitionForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    if (empty($petition) || ($petition->bundle() != 'petition')) {
      throw new \Exception("Error, current node is not a petition");
    }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($flag) {
      ...
      //$petition_link = \Drupal::service('path.alias_manager')->getAliasByPath('/node/' . $petition->id());
      ...
      $result = $mailManager->mail('spn', 'petition_pending_confirmation', $to, $language_code, $params);
      if ($result['result'] !== true) {
        \Drupal::messenger()->addError($this->t('There was a problem sending your message and it was not sent.'));
      }
      else {
        \Drupal::messenger()->addMessage($this->t('@emp_name, A confirmation link has been sent to you by email to the following adress: @mail', array('@emp_name' => $form_state->getValue('name'), '@mail'=>$user_mail)));
      }

    // if signature already exists alert the user
    } else {
      \Drupal::messenger()->addMessage($this->t('@emp_name, You have already signed this petition!', array('@emp_name' => $form_state->getValue('name'))));
    }
